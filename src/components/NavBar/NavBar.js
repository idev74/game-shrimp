...
export default function NavBar() {
    ...
    window.onscroll = function () {
        if (document.body.scrollTop >= 400 || document.documentElement.scrollTop >= 400) {
            nav.classList.add("bg-yellow-300");
            nav.classList.remove("bg-transparent");
            setStroke("teal");
        } else {
            nav.classList.add("bg-transparent");
            nav.classList.remove("bg-yellow-300");
    ...
    return (
        <section className="my-7 sticky top-0">
            <nav id="nav" className="flex flex-row p-3 mt-auto mb-auto justify-center">
                <NavLink
                    className="inline-block rounded py-2 px-4 bg-cyan-500 hover:bg-purple-700 text-white outline-double outline-3 active:outline-offset-4 mx-5"
                    to="/">Home</NavLink>
                <NavLink
                    className="inline-block rounded py-2 px-4 bg-cyan-500 hover:bg-purple-700 text-white outline-double outline-3 active:outline-offset-4 mx-5"
